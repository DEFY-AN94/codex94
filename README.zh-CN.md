...
以下命令安装已发布的稳定 `v0.1.7` 源码，不会安装尚未发布的 `0.1.8` 候选版：
...
本节描述当前 `0.1.8` 开发候选版；以下标为“候选版新增”的功能不包含在上面的
稳定 `v0.1.7` 安装中。

- 候选版新增：在 Dashboard → 显示中选择**圆环 + 百分比**、**仅百分比**或
  **仅圆环**。保存的布局在下次启动 Codex94 时生效，不会即时改变正在运行的
  菜单栏宽度；状态标记位于圆环中央，或在“仅百分比”模式下占用固定尾部位置。
- 候选版新增：分别自定义充足（50–100%）、偏低（20–49%）、紧张（0–19%）
...
- 候选版新增：额度行下方独立显示绝对**重置时间**，包含完整日期、小时/分钟及
...
- 候选版新增：错误横幅提供**打开连接设置**或**打开诊断**，复用同一个 Dashboard
...
- 将额度严重度与连接/数据新鲜度分开：额度圆环、百分比和进度条使用同一套解析后
  ...
  蓝色/青色连接强调色。候选版在没有可用数据且连接不可用时，标记、横幅和
...
界面选项（包括菜单栏额度偏好）和用户手动选择的可执行文件路径。候选版新增
